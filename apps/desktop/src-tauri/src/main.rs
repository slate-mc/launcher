...
#[tauri::command]
async fn storage_clear_category(
    state: tauri::State<'_, DesktopState>,
...
#[tauri::command]
async fn trashed_instance_restore(
    state: tauri::State<'_, DesktopState>,
...
#[tauri::command]
async fn trashed_instance_delete(
    state: tauri::State<'_, DesktopState>,
...
#[tauri::command]
async fn trashed_instances_empty(
    state: tauri::State<'_, DesktopState>,
...
fn storage_category(category: StorageCategoryDto, usage: DiskUsage) -> StorageCategorySummary {
...
fn storage_scan_error() -> AppError {
    AppError::new(
...
fn storage_cleanup_error() -> AppError {
    AppError::new(
...
async fn rollback_staged_deletions(staged: Vec<StagedInstanceDeletion>) {
    let _ = tauri::async_runtime::spawn_blocking(move || {
        for item in staged.into_iter().rev() {
            let _ = item.rollback();
        }
...
async fn purge_staged_deletions(staged: Vec<StagedInstanceDeletion>) {
    let _ = tauri::async_runtime::spawn_blocking(move || {
        for item in staged {
            let _ = item.purge();
        }
...
async fn purge_expired_instance_trash(state: DesktopState) {
