...
impl MinecraftAuthClient {
    ...
    async fn xsts_token(&self, user_token: &SecretString) -> Result<XboxToken, AuthError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(xerr.map_or(AuthError::StageRejected {
    ...
    async fn minecraft_profile(
        ...
        Ok(MinecraftProfile {
            id,
            name: payload.name,
            skin_url: payload.skins.into_iter().find_map(|skin| {
                Url::parse(&skin.url)
                    .ok()
...
async fn parse_oauth_response(response: reqwest::Response) -> Result<OAuthToken, AuthError> {
    if !response.status().is_success() {
        ...
        return Err(code.map_or(AuthError::StageRejected {
...
fn parse_callback_target(target: &str, expected_state: &str) -> Result<String, AuthError> {
    ...
    let code = query.get("code").ok_or(AuthError::AuthorizationCodeMissing)?;
...
mod tests {
    use super::{
        AuthError, MinecraftAuthClient, MinecraftAuthConfig, parse_callback_target,
        random_url_secret,
    };
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (start, pending) = client.begin_login().await?;
        let query = start
            .authorization_url
            .query_pairs()
