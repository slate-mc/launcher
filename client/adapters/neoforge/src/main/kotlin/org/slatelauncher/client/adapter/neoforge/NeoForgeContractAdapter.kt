...
public class NeoForgeContractAdapter(
    private val minecraftVersion: String,
    private val loaderVersion: String,
) : LoaderAdapter {
    override val loader: LoaderKind = LoaderKind.NEOFORGE

    override fun handshake(
        target: GameTarget,
        modules: Collection<ModuleDescriptor>,
    ): ClientHandshake {
        require(target.loader == loader) { "NeoForge adapter cannot serve ${target.loader}" }
        require(
            target.minecraftVersion == minecraftVersion && target.loaderVersion == loaderVersion,
        ) {
            "NeoForge adapter target does not match its exact build target"
        }
        require(modules.all { it.supports(target) }) {
            "one or more Slate Client modules do not support this NeoForge target"
        }
