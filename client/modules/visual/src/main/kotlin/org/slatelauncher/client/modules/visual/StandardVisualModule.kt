...
public class StandardVisualModule(target: ModuleTarget) : ClientModule {
    override val descriptor: ModuleDescriptor =
        ModuleDescriptor(
            id = ModuleId.parse("slate.visual"),
            version = SemanticVersion(0, 1, 0),
            capabilities = setOf(ModuleCapability.VISUAL),
            supportedTargets = setOf(target),
        )

    override fun start(context: ModuleContext) {
        context.events.emit("module.visual.started", "visual preferences applied")
    }

    override fun stop(context: ModuleContext) {
        context.events.emit("module.visual.stopped", "visual module stopped")
    }
}
